...
def world_to_pixel(pos, env_size=512, world_min=0, world_max=512, swap_xy=False, flip_y=True):
    """
    ...
    Args:
        pos: [x, y] in world coordinates
        env_size: Size of the rendered image (default 512)
        world_min: Minimum world coordinate (default 0)
        world_max: Maximum world coordinate (default 512 for PushT)
        swap_xy: If True, swap x and y
        flip_y: If True, flip y-axis (for image coordinates where y=0 is at top)
    """
    if swap_xy:
        x_world, y_world = pos[1], pos[0]
    else:
        x_world, y_world = pos[0], pos[1]
    
    x_pixel = int((x_world - world_min) / (world_max - world_min) * env_size)
    if flip_y:
        y_pixel = int((1.0 - (y_world - world_min) / (world_max - world_min)) * env_size)  # Flip y-axis
    else:
        y_pixel = int((y_world - world_min) / (world_max - world_min) * env_size)
